...
> **[반려됨, 2026-07-19]** 이 방향으로 가지 않기로 함. 헬스체크를 메인 트래픽과 분리된
> 포트/스레드풀로 격리하는 것은 "부하가 몰릴 때 서버가 헬스체크에 응답 못 하는 상태"를
> 문제로 보고 회피하는 접근인데, 이 상태는 실제로 unhealthy한 게 맞다 — 서버가 요청을
> 처리 못 하고 있으면 ALB가 내려버리는 게 정상 동작이다. 증상(헬스체크 실패)을 안 보이게
> 만들 게 아니라, 그 부하 상황에서 왜 요청 처리가 그렇게까지 밀리는지 근본 원인을 찾아서
> 고치는 방향으로 간다. 아래 내용은 폐기된 계획으로, 참고용으로만 남겨둔다.
